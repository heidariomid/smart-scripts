...
- **Tool execution** — `run_claude()` runs `claude -p <prompt> --output-format text --tools <list> --disallowedTools Write Edit NotebookEdit Bash`. By default `<list>` is `""` (no tools), so the model can't write files (no permission prompts) and returns the result on stdout, which the server saves. Per-agent exceptions live in `MODE_TOOLS`: agents whose prompts need read-only web access get those tools enabled (e.g. `travel-guide` → `WebSearch,WebFetch`, so it can verify places and build real links just like it does as a Claude Code agent). `--disallowedTools` always denies write/edit/bash, so even tool-enabled agents can't write files or run shell.
